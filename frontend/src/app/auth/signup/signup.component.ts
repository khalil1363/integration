...
export class SignupComponent implements AfterViewInit, OnDestroy {
  ...
  avatarPreview = '';
  /** Inline placeholder when no photo selected (avoids via.placeholder.com) */
  avatarPlaceholder = PLACEHOLDERS.avatar100;
    ...
    this.authService.signup(payload).subscribe({
      ...
      error: err => {
        ...
        if (status === 413) {
          ...
          this.signupError = 'Impossible de joindre le serveur. Vérifiez que la gateway (8080) et le microservice users (8011) sont démarrés, puis relancez avec « ng serve ».';
